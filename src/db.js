const { AsyncLocalStorage } = require("node:async_hooks");
const { DatabaseSync } = require("node:sqlite");

const { Pool } = require("pg");

const {
  ROOT_DIR,
  PUBLIC_DIR,
  DATA_DIR,
  DB_PATH,
  UPLOADS_DIR,
  VIDEOS_DIR,
  MATERIALS_DIR,
  DATABASE_URL,
  DATABASE_SSL,
  isPostgres,
} = require("./config");

const SQLITE_SCHEMA = `
  CREATE TABLE IF NOT EXISTS users (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    name TEXT NOT NULL,
    email TEXT NOT NULL UNIQUE,
    mobile TEXT UNIQUE,
    exam_preparing_for TEXT DEFAULT '',
    password_hash TEXT NOT NULL,
    role TEXT NOT NULL CHECK (role IN ('student', 'instructor', 'admin')),
    email_verified_at TEXT,
    mobile_verified_at TEXT,
    created_at TEXT NOT NULL
  );

  CREATE TABLE IF NOT EXISTS signup_verifications (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    name TEXT NOT NULL,
    email TEXT NOT NULL,
    mobile TEXT NOT NULL,
    exam_preparing_for TEXT NOT NULL,
    role TEXT NOT NULL DEFAULT 'student' CHECK (role IN ('student', 'instructor', 'admin')),
    password_hash TEXT NOT NULL,
    otp_hash TEXT NOT NULL,
    attempt_count INTEGER NOT NULL DEFAULT 0,
    created_at TEXT NOT NULL,
    updated_at TEXT NOT NULL,
    expires_at TEXT NOT NULL,
    verified_at TEXT
  );

  CREATE TABLE IF NOT EXISTS courses (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    title TEXT NOT NULL,
    description TEXT NOT NULL,
    category TEXT NOT NULL,
    intro_text TEXT DEFAULT '',
    instructor_id INTEGER NOT NULL,
    status TEXT NOT NULL DEFAULT 'pending' CHECK (status IN ('pending', 'approved', 'rejected')),
    created_at TEXT NOT NULL,
    updated_at TEXT NOT NULL,
    FOREIGN KEY (instructor_id) REFERENCES users(id) ON DELETE CASCADE
  );

  CREATE TABLE IF NOT EXISTS course_pricing (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    course_id INTEGER NOT NULL UNIQUE,
    price_amount INTEGER NOT NULL DEFAULT 0,
    currency TEXT NOT NULL DEFAULT 'INR',
    updated_at TEXT NOT NULL,
    FOREIGN KEY (course_id) REFERENCES courses(id) ON DELETE CASCADE
  );

  CREATE TABLE IF NOT EXISTS lessons (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    course_id INTEGER NOT NULL,
    title TEXT NOT NULL,
    summary TEXT DEFAULT '',
    position INTEGER NOT NULL DEFAULT 1,
    video_type TEXT NOT NULL DEFAULT 'none' CHECK (video_type IN ('none', 'external', 'upload')),
    video_url TEXT DEFAULT '',
    material_path TEXT DEFAULT '',
    created_at TEXT NOT NULL,
    FOREIGN KEY (course_id) REFERENCES courses(id) ON DELETE CASCADE
  );

  CREATE TABLE IF NOT EXISTS enrollments (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    user_id INTEGER NOT NULL,
    course_id INTEGER NOT NULL,
    enrolled_at TEXT NOT NULL,
    last_lesson_id INTEGER,
    UNIQUE (user_id, course_id),
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
    FOREIGN KEY (course_id) REFERENCES courses(id) ON DELETE CASCADE,
    FOREIGN KEY (last_lesson_id) REFERENCES lessons(id) ON DELETE SET NULL
  );

  CREATE TABLE IF NOT EXISTS lesson_progress (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    user_id INTEGER NOT NULL,
    course_id INTEGER NOT NULL,
    lesson_id INTEGER NOT NULL,
    completed_at TEXT NOT NULL,
    UNIQUE (user_id, lesson_id),
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
    FOREIGN KEY (course_id) REFERENCES courses(id) ON DELETE CASCADE,
    FOREIGN KEY (lesson_id) REFERENCES lessons(id) ON DELETE CASCADE
  );

  CREATE TABLE IF NOT EXISTS quizzes (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    course_id INTEGER NOT NULL UNIQUE,
    title TEXT NOT NULL,
    instructions TEXT DEFAULT '',
    created_at TEXT NOT NULL,
    FOREIGN KEY (course_id) REFERENCES courses(id) ON DELETE CASCADE
  );

  CREATE TABLE IF NOT EXISTS questions (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    quiz_id INTEGER NOT NULL,
    prompt TEXT NOT NULL,
    option_a TEXT NOT NULL,
    option_b TEXT NOT NULL,
    option_c TEXT NOT NULL,
    option_d TEXT NOT NULL,
    correct_option TEXT NOT NULL CHECK (correct_option IN ('a', 'b', 'c', 'd')),
    FOREIGN KEY (quiz_id) REFERENCES quizzes(id) ON DELETE CASCADE
  );

  CREATE TABLE IF NOT EXISTS quiz_attempts (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    user_id INTEGER NOT NULL,
    quiz_id INTEGER NOT NULL,
    score INTEGER NOT NULL,
    total INTEGER NOT NULL,
    answers_json TEXT NOT NULL,
    submitted_at TEXT NOT NULL,
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
    FOREIGN KEY (quiz_id) REFERENCES quizzes(id) ON DELETE CASCADE
  );

  CREATE TABLE IF NOT EXISTS notifications (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    user_id INTEGER,
    role_target TEXT,
    title TEXT NOT NULL,
    message TEXT NOT NULL,
    link TEXT DEFAULT '',
    created_at TEXT NOT NULL,
    read_at TEXT,
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
  );

  CREATE TABLE IF NOT EXISTS payment_orders (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    user_id INTEGER NOT NULL,
    course_id INTEGER NOT NULL,
    amount INTEGER NOT NULL,
    currency TEXT NOT NULL DEFAULT 'INR',
    status TEXT NOT NULL DEFAULT 'pending' CHECK (status IN ('pending', 'paid', 'failed')),
    gateway TEXT NOT NULL DEFAULT 'sandbox',
    gateway_order_id TEXT DEFAULT '',
    gateway_payment_id TEXT DEFAULT '',
    gateway_signature TEXT DEFAULT '',
    gateway_receipt TEXT DEFAULT '',
    payment_method TEXT DEFAULT '' CHECK (payment_method IN ('', 'card', 'upi', 'netbanking')),
    payment_descriptor TEXT DEFAULT '',
    payer_name TEXT DEFAULT '',
    payer_email TEXT DEFAULT '',
    transaction_reference TEXT DEFAULT '',
    failure_reason TEXT DEFAULT '',
    created_at TEXT NOT NULL,
    updated_at TEXT NOT NULL,
    paid_at TEXT,
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
    FOREIGN KEY (course_id) REFERENCES courses(id) ON DELETE CASCADE
  );

  CREATE TABLE IF NOT EXISTS live_classes (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    title TEXT NOT NULL,
    description TEXT NOT NULL,
    category TEXT NOT NULL,
    host_id INTEGER NOT NULL,
    meeting_url TEXT NOT NULL,
    scheduled_at TEXT NOT NULL,
    duration_minutes INTEGER NOT NULL DEFAULT 60,
    created_at TEXT NOT NULL,
    updated_at TEXT NOT NULL,
    FOREIGN KEY (host_id) REFERENCES users(id) ON DELETE CASCADE
  );

  CREATE TABLE IF NOT EXISTS live_class_registrations (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    live_class_id INTEGER NOT NULL,
    user_id INTEGER NOT NULL,
    registered_at TEXT NOT NULL,
    UNIQUE (live_class_id, user_id),
    FOREIGN KEY (live_class_id) REFERENCES live_classes(id) ON DELETE CASCADE,
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
  );
`;

const POSTGRES_SCHEMA = `
  CREATE TABLE IF NOT EXISTS users (
    id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    name TEXT NOT NULL,
    email TEXT NOT NULL UNIQUE,
    mobile TEXT UNIQUE,
    exam_preparing_for TEXT DEFAULT '',
    password_hash TEXT NOT NULL,
    role TEXT NOT NULL CHECK (role IN ('student', 'instructor', 'admin')),
    email_verified_at TEXT,
    mobile_verified_at TEXT,
    created_at TEXT NOT NULL
  );

  CREATE TABLE IF NOT EXISTS signup_verifications (
    id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    name TEXT NOT NULL,
    email TEXT NOT NULL,
    mobile TEXT NOT NULL,
    exam_preparing_for TEXT NOT NULL,
    role TEXT NOT NULL DEFAULT 'student' CHECK (role IN ('student', 'instructor', 'admin')),
    password_hash TEXT NOT NULL,
    otp_hash TEXT NOT NULL,
    attempt_count INTEGER NOT NULL DEFAULT 0,
    created_at TEXT NOT NULL,
    updated_at TEXT NOT NULL,
    expires_at TEXT NOT NULL,
    verified_at TEXT
  );

  CREATE TABLE IF NOT EXISTS courses (
    id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    title TEXT NOT NULL,
    description TEXT NOT NULL,
    category TEXT NOT NULL,
    intro_text TEXT DEFAULT '',
    instructor_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    status TEXT NOT NULL DEFAULT 'pending' CHECK (status IN ('pending', 'approved', 'rejected')),
    created_at TEXT NOT NULL,
    updated_at TEXT NOT NULL
  );

  CREATE TABLE IF NOT EXISTS course_pricing (
    id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    course_id INTEGER NOT NULL UNIQUE REFERENCES courses(id) ON DELETE CASCADE,
    price_amount INTEGER NOT NULL DEFAULT 0,
    currency TEXT NOT NULL DEFAULT 'INR',
    updated_at TEXT NOT NULL
  );

  CREATE TABLE IF NOT EXISTS lessons (
    id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    course_id INTEGER NOT NULL REFERENCES courses(id) ON DELETE CASCADE,
    title TEXT NOT NULL,
    summary TEXT DEFAULT '',
    position INTEGER NOT NULL DEFAULT 1,
    video_type TEXT NOT NULL DEFAULT 'none' CHECK (video_type IN ('none', 'external', 'upload')),
    video_url TEXT DEFAULT '',
    material_path TEXT DEFAULT '',
    created_at TEXT NOT NULL
  );

  CREATE TABLE IF NOT EXISTS enrollments (
    id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    course_id INTEGER NOT NULL REFERENCES courses(id) ON DELETE CASCADE,
    enrolled_at TEXT NOT NULL,
    last_lesson_id INTEGER REFERENCES lessons(id) ON DELETE SET NULL,
    UNIQUE (user_id, course_id)
  );

  CREATE TABLE IF NOT EXISTS lesson_progress (
    id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    course_id INTEGER NOT NULL REFERENCES courses(id) ON DELETE CASCADE,
    lesson_id INTEGER NOT NULL REFERENCES lessons(id) ON DELETE CASCADE,
    completed_at TEXT NOT NULL,
    UNIQUE (user_id, lesson_id)
  );

  CREATE TABLE IF NOT EXISTS quizzes (
    id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    course_id INTEGER NOT NULL UNIQUE REFERENCES courses(id) ON DELETE CASCADE,
    title TEXT NOT NULL,
    instructions TEXT DEFAULT '',
    created_at TEXT NOT NULL
  );

  CREATE TABLE IF NOT EXISTS questions (
    id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    quiz_id INTEGER NOT NULL REFERENCES quizzes(id) ON DELETE CASCADE,
    prompt TEXT NOT NULL,
    option_a TEXT NOT NULL,
    option_b TEXT NOT NULL,
    option_c TEXT NOT NULL,
    option_d TEXT NOT NULL,
    correct_option TEXT NOT NULL CHECK (correct_option IN ('a', 'b', 'c', 'd'))
  );

  CREATE TABLE IF NOT EXISTS quiz_attempts (
    id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    quiz_id INTEGER NOT NULL REFERENCES quizzes(id) ON DELETE CASCADE,
    score INTEGER NOT NULL,
    total INTEGER NOT NULL,
    answers_json TEXT NOT NULL,
    submitted_at TEXT NOT NULL
  );

  CREATE TABLE IF NOT EXISTS notifications (
    id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    user_id INTEGER REFERENCES users(id) ON DELETE CASCADE,
    role_target TEXT,
    title TEXT NOT NULL,
    message TEXT NOT NULL,
    link TEXT DEFAULT '',
    created_at TEXT NOT NULL,
    read_at TEXT
  );

  CREATE TABLE IF NOT EXISTS payment_orders (
    id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    course_id INTEGER NOT NULL REFERENCES courses(id) ON DELETE CASCADE,
    amount INTEGER NOT NULL,
    currency TEXT NOT NULL DEFAULT 'INR',
    status TEXT NOT NULL DEFAULT 'pending' CHECK (status IN ('pending', 'paid', 'failed')),
    gateway TEXT NOT NULL DEFAULT 'sandbox',
    gateway_order_id TEXT DEFAULT '',
    gateway_payment_id TEXT DEFAULT '',
    gateway_signature TEXT DEFAULT '',
    gateway_receipt TEXT DEFAULT '',
    payment_method TEXT DEFAULT '' CHECK (payment_method IN ('', 'card', 'upi', 'netbanking')),
    payment_descriptor TEXT DEFAULT '',
    payer_name TEXT DEFAULT '',
    payer_email TEXT DEFAULT '',
    transaction_reference TEXT DEFAULT '',
    failure_reason TEXT DEFAULT '',
    created_at TEXT NOT NULL,
    updated_at TEXT NOT NULL,
    paid_at TEXT
  );

  CREATE TABLE IF NOT EXISTS live_classes (
    id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    title TEXT NOT NULL,
    description TEXT NOT NULL,
    category TEXT NOT NULL,
    host_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    meeting_url TEXT NOT NULL,
    scheduled_at TEXT NOT NULL,
    duration_minutes INTEGER NOT NULL DEFAULT 60,
    created_at TEXT NOT NULL,
    updated_at TEXT NOT NULL
  );

  CREATE TABLE IF NOT EXISTS live_class_registrations (
    id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    live_class_id INTEGER NOT NULL REFERENCES live_classes(id) ON DELETE CASCADE,
    user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    registered_at TEXT NOT NULL,
    UNIQUE (live_class_id, user_id)
  );
`;

let db = null;
let pool = null;
const transactionStore = new AsyncLocalStorage();

if (isPostgres) {
  if (!DATABASE_URL) {
    throw new Error("DATABASE_URL is required when DATABASE_PROVIDER=postgres.");
  }

  pool = new Pool({
    connectionString: DATABASE_URL,
    ssl: DATABASE_SSL || undefined,
  });

  pool.on("error", (error) => {
    console.error("Unexpected Postgres pool error:", error);
  });
} else {
  db = new DatabaseSync(DB_PATH);
  db.exec("PRAGMA foreign_keys = ON;");
  db.exec("PRAGMA journal_mode = WAL;");
}

function nowIso() {
  return new Date().toISOString();
}

function compileNamedQuery(sql, params = {}) {
  const values = [];
  const statement = sql.replace(/:([a-zA-Z_][a-zA-Z0-9_]*)/g, (_match, key) => {
    values.push(Object.prototype.hasOwnProperty.call(params, key) ? params[key] : null);
    return `$${values.length}`;
  });

  return {
    statement,
    values,
  };
}

function appendReturningId(sql) {
  if (!/^\s*insert\b/i.test(sql) || /\breturning\b/i.test(sql)) {
    return sql;
  }

  return `${sql.trim().replace(/;$/, "")} RETURNING id`;
}

function extractAliases(sql) {
  const aliases = new Map();
  const pattern = /\bAS\s+("?)([a-zA-Z_][a-zA-Z0-9_]*)\1/gi;
  let match = pattern.exec(sql);

  while (match) {
    aliases.set(match[2].toLowerCase(), match[2]);
    match = pattern.exec(sql);
  }

  return aliases;
}

function normalizePostgresRow(row, aliases) {
  if (!row || aliases.size === 0) {
    return row;
  }

  const normalized = { ...row };
  for (const [key, value] of Object.entries(row)) {
    const alias = aliases.get(key.toLowerCase());
    if (alias && alias !== key) {
      normalized[alias] = value;
    }
  }

  return normalized;
}

async function executePostgres(sql, params = {}, { mode } = {}) {
  const executor = transactionStore.getStore() || pool;
  const compiled = compileNamedQuery(mode === "run" ? appendReturningId(sql) : sql, params);
  const result = await executor.query(compiled.statement, compiled.values);
  const aliases = extractAliases(sql);

  if (mode === "run") {
    return {
      rowCount: result.rowCount,
      lastInsertRowid: result.rows[0]?.id ?? 0,
    };
  }

  if (mode === "get") {
    return normalizePostgresRow(result.rows[0], aliases) || null;
  }

  return result.rows.map((row) => normalizePostgresRow(row, aliases));
}

async function run(sql, params = {}) {
  if (isPostgres) {
    return executePostgres(sql, params, { mode: "run" });
  }

  return db.prepare(sql).run(params);
}

async function get(sql, params = {}) {
  if (isPostgres) {
    return executePostgres(sql, params, { mode: "get" });
  }

  return db.prepare(sql).get(params);
}

async function all(sql, params = {}) {
  if (isPostgres) {
    return executePostgres(sql, params, { mode: "all" });
  }

  return db.prepare(sql).all(params);
}

async function withTransaction(work) {
  if (isPostgres) {
    const client = await pool.connect();

    try {
      await client.query("BEGIN");
      const result = await transactionStore.run(client, async () => work());
      await client.query("COMMIT");
      return result;
    } catch (error) {
      await client.query("ROLLBACK");
      throw error;
    } finally {
      client.release();
    }
  }

  db.exec("BEGIN");
  try {
    const result = await work();
    db.exec("COMMIT");
    return result;
  } catch (error) {
    db.exec("ROLLBACK");
    throw error;
  }
}

function text(value) {
  return typeof value === "string" ? value.trim() : "";
}

function lowerEmail(value) {
  return text(value).toLowerCase();
}

function toInt(value) {
  const parsed = Number.parseInt(value, 10);
  return Number.isFinite(parsed) ? parsed : null;
}

function isRole(value) {
  return ["student", "instructor", "admin"].includes(value);
}

function isStatus(value) {
  return ["pending", "approved", "rejected"].includes(value);
}

function slugify(value) {
  return (
    text(value)
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, "-")
      .replace(/^-+|-+$/g, "")
      .slice(0, 48) || "file"
  );
}

async function initializeDatabase() {
  if (isPostgres) {
    await pool.query(POSTGRES_SCHEMA);
    await ensureUserSchema();
    await ensurePaymentOrderSchema();
    return;
  }

  db.exec(SQLITE_SCHEMA);
  await ensureUserSchema();
  await ensurePaymentOrderSchema();
}

async function hasColumn(tableName, columnName) {
  if (isPostgres) {
    const row = await get(
      `
        SELECT 1 AS present
        FROM information_schema.columns
        WHERE table_schema = 'public'
          AND table_name = :tableName
          AND column_name = :columnName
      `,
      {
        tableName,
        columnName,
      },
    );

    return Boolean(row?.present);
  }

  const rows = db.prepare(`PRAGMA table_info(${tableName})`).all();
  return rows.some((row) => String(row.name || "").toLowerCase() === String(columnName).toLowerCase());
}

async function ensureColumn(tableName, columnName, definition) {
  if (await hasColumn(tableName, columnName)) {
    return;
  }

  await run(`ALTER TABLE ${tableName} ADD COLUMN ${columnName} ${definition}`);
}

async function ensureIndex(statement) {
  if (isPostgres) {
    await pool.query(statement);
    return;
  }

  db.exec(statement);
}

async function ensureUserSchema() {
  await ensureColumn("users", "mobile", "TEXT");
  await ensureColumn("users", "exam_preparing_for", "TEXT DEFAULT ''");
  await ensureColumn("users", "email_verified_at", "TEXT");
  await ensureColumn("users", "mobile_verified_at", "TEXT");

  await ensureIndex("CREATE UNIQUE INDEX IF NOT EXISTS users_mobile_unique ON users(mobile)");
  await ensureIndex("CREATE INDEX IF NOT EXISTS signup_verifications_email_idx ON signup_verifications(email)");
  await ensureIndex("CREATE INDEX IF NOT EXISTS signup_verifications_mobile_idx ON signup_verifications(mobile)");
}

async function ensurePaymentOrderSchema() {
  await ensureColumn("payment_orders", "gateway", "TEXT NOT NULL DEFAULT 'sandbox'");
  await ensureColumn("payment_orders", "gateway_order_id", "TEXT DEFAULT ''");
  await ensureColumn("payment_orders", "gateway_payment_id", "TEXT DEFAULT ''");
  await ensureColumn("payment_orders", "gateway_signature", "TEXT DEFAULT ''");
  await ensureColumn("payment_orders", "gateway_receipt", "TEXT DEFAULT ''");
}

module.exports = {
  ROOT_DIR,
  PUBLIC_DIR,
  DATA_DIR,
  DB_PATH,
  UPLOADS_DIR,
  VIDEOS_DIR,
  MATERIALS_DIR,
  db,
  pool,
  nowIso,
  run,
  get,
  all,
  withTransaction,
  text,
  lowerEmail,
  toInt,
  isRole,
  isStatus,
  slugify,
  initializeDatabase,
  isPostgres,
};
